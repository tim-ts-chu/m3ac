...
class DiscriminateAlgorithm:
    ...
    def optimize_agent(self, batch_size, step: int) -> Dict:
        optim_info = {}
        real_samples = self._real_buffer.sample(batch_size)
        imag_samples = self._imag_buffer.sample(batch_size)
        samples = {}
        for k in BufferFields.keys():
            samples[k] = torch.cat((real_samples[k], imag_samples[k]), dim=0)
        labels = torch.cat((torch.ones(batch_size), torch.zeros(batch_size))).view(-1,1)
    ...
    def optimize_model_agent(self, batch_size):
        optim_info = {}
        real_samples = self._real_buffer.sample(batch_size)
        ...
        labels = torch.ones(batch_size).view(-1,1)
        loss = torch.nn.BCEWithLogitsLoss()(logits, labels)
        loss.backward()
